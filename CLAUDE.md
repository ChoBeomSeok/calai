...
`calai.kr` — 가입·로그인 없이 브라우저에서 즉시 계산하는 한국어 도구·계산기 102개 모음 사이트. Next.js 16 + React 19 + Tailwind v4. 모든 계산은 클라이언트에서 수행되어 서버로 입력값이 전송되지 않는다 (예외: `/api/exchange`, `/api/coin` 시세 프록시).
...
- **102개 도구 페이지 전부 `"use client"`** — 계산은 모두 브라우저에서. 서버 컴포넌트로 바꾸지 말 것 (interactive 폼).
