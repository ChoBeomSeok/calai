...
export default function RemoveBackgroundPage() {
  ...
  // 모델 lazy 로딩: 페이지 진입 시 자동 다운로드 하지 않고, "누끼 따기 시작" 클릭 시점에만 로딩
  // 메모리·네트워크 절약. 사용자가 선택 안 한 모델은 절대 받지 않음.
        ...
        {file && status === "idle" && (
          <div className="space-y-4">
            ...
            <details open className="rounded-xl border border-slate-200 dark:border-slate-700 group">
              ...
              <div className="px-4 pb-4 pt-1 space-y-4 text-sm">
                ...
                <div className="text-[11px] text-slate-400 flex items-center gap-2">
                  ...
                  모델은 <strong>&quot;✨ 누끼 따기 시작&quot;</strong> 클릭 시점에 다운로드됩니다 ({MODEL_INFO[model].size})
                </div>
              </div>
            </details>
            ...
            <div className="text-xs text-slate-500 dark:text-slate-400 text-center">
              선택한 모델 <strong>{MODEL_INFO[model].label}</strong> ({MODEL_INFO[model].size}) — 클릭 시 다운로드 후 처리. 첫 사용 이후엔 브라우저 캐시.
            </div>
          </div>
        )}
